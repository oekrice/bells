...
class IZNodeGene(BaseGene):
    ...
    _gene_attributes = [FloatAttribute("bias"), FloatAttribute("a"), FloatAttribute("b"), FloatAttribute("c"), FloatAttribute("d")]

    def distance(self, other, config):
        s = abs(self.a - other.a) + abs(self.b - other.b) + abs(self.c - other.c) + abs(self.d - other.d)
...
class IZNN(object):
    ...
    def set_inputs(self, inputs):
        ...
        if len(inputs) != len(self.inputs):
            raise RuntimeError(
                "Number of inputs {0:d} does not match number of input nodes {1:d}".format(len(inputs), len(self.inputs))
            )
